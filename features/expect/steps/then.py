...
def the_expect_object_will_use_our_actual_data(context):
    Expect(context.result.actual_data).to_be(context.actual_data)
...
def the_expectation_should_pass(context):
    Expect(type(context.result)).not_.to_be(ExpectationException)


@then
def the_expectation_should_fail(context):
    Expect(type(context.result)).to_be(ExpectationException)
